...
def full_features_factory(model, functional_operators):
    features_by_operator = tuple(
        features_factory(model, fo) for fo in functional_operators
    )
    ...
    def full_features(params, batch_samples):
        return jnp.concatenate(
            tuple(
                fbo(params, bs).T
                for fbo, bs in zip(features_by_operator, batch_samples)
            ),
            axis=1,
        )
...
def nat_grad_factory(
    full_features, true_gradient, const_tol=None, const_cut_low_signal=True
):
    def natural_gradient(
        params, batch_samples, tol=const_tol, cut_low_signal=const_cut_low_signal
    ):
        if tol is None:
            raise ValueError("tol should be specified")
        full_features_evaluated = full_features(params, batch_samples)

        U_features, Lambda_features, VT_features = jax.scipy.linalg.svd(
            full_features_evaluated, full_matrices=False
        )
        tol_selector = Lambda_features > tol
        U_features, Lambda_features, VT_features = (
            U_features[:, tol_selector],
            Lambda_features[tol_selector],
            VT_features[tol_selector],
        )
        ...
        if not cut_low_signal:
            low_signal_projection = (
                true_gradient_evaluated - VT_features.T @ gradient_rotated
            )
...
def quadratic_gradient_factory(model, functional_operators, sources=None):
    operators_on_model = tuple(
        make_operator_on_model(model, fo) for fo in functional_operators
    )
    ...
    def full_gradient(params, batch_samples):
        return jnp.concatenate(
            tuple(
                voom(params, bs) - vs(bs)
                for voom, vs, bs in zip(v_operators_on_model, v_sources, batch_samples)
            ),
            axis=0,
        )
...
def quadratic_nat_grad_factory(
    model, functional_operators, const_tol, sources=None, const_cut_low_signal=True
):
    ...
    return nat_grad_factory(
        full_features, true_gradient, const_tol, const_cut_low_signal
    )
