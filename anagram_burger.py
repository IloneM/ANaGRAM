...
expe_parameters.layer_sizes = [2, 64, 64, 1]
...
interior_sample = jnp.array(tuple((t,x) for t in jnp.linspace(0.,1., ep.n_inner_samples+2)[1:-1] for x in jnp.linspace(-1.,1., ep.n_inner_samples+2)[1:-1]))
initial_sample = jnp.stack((jnp.zeros(ep.n_boundary_samples), jnp.linspace(-1.,1., ep.n_boundary_samples)), axis=1)
