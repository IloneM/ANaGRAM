...
from anagram import quadratic_nat_grad_factory, identity_operator, null_source
...
integrator_samples = tuple(make_integrator_sample(integrator) for integrator in (lboundary_integrator, rboundary_integrator, initial_integrator, interior_integrator))

# constructor of the anagram natural gradient
## THIS VALUE CAN BE OPTIMIZED
rcond = 1e-6
nat_grad = quadratic_nat_grad_factory(model, (identity_operator, identity_operator, identity_operator, heat_operator), rcond, (null_source, null_source, u_0, null_source))
