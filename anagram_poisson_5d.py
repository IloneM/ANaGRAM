...
from anagram import identity_operator, laplacian, quadratic_nat_grad_factory
from jax import grad, jit, random, vmap
from ngrad.domains import HypercubeBoundary, Hyperrectangle
...
# domains
dim = 5
interior = Hyperrectangle([(0.0, 1.0) for _ in range(dim)])
...
def interior_loss(params):
    ...
    return 0.5 * interior_integrator(
        vmap(lambda x: (laplace_model(x) - f(x)) ** 2, (0)),
    )
...
def boundary_loss(params):
    return 0.5 * boundary_integrator(
        lambda x: (v_model(params, x) - v_u_star(x)) ** 2.0,
    )
...
integrator_samples = tuple(
    ...
    for integrator in (boundary_integrator, interior_integrator)
)
...
nat_grad = quadratic_nat_grad_factory(
    model,
    (identity_operator, laplace_operator),
    (u_star, f),
    rcond,
)
